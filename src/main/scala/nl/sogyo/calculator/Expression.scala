...
sealed trait Expression {
  ...
  def value(vars: Map[String, Double] = Map()): Try[Double]
...
case class Function(func: (Double => Double), stringRepresentation: String, expr: Expression) extends Expression {
  def value(vars: Map[String, Double]): Try[Double] = for (e <- expr.value(vars)) yield func(e)
...
case class Number(num: Double) extends Expression {
  def value(vars: Map[String, Double]): Try[Double] = Success(num)
  override def toString = num.toString
}

case class Plus(left: Expression, right: Expression) extends Expression with BinaryOperator {
  def value(vars: Map[String, Double]): Try[Double] = for (l <- left.value(vars); r <- right.value(vars)) yield l + r
...
case class Power(left: Expression, right: Expression) extends Expression with BinaryOperator {
  def value(vars: Map[String, Double]): Try[Double] = for (l <- left.value(vars); r <- right.value(vars)) yield math.pow(l, r)
  override def toString = super.toString + "^"
}

case class Variable(name: String) extends Expression {
  def value(vars: Map[String, Double]): Try[Double] = vars.get(name) match {
...
case object Empty extends Expression {
  def value(vars: Map[String, Double]): Try[Double] = Failure(new exceptions.EmptyExpressionValueException("An empty expression has no value!"))
