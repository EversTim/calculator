...
class ExpressionSpec extends FlatSpec with Matchers {
  ...
  it should "return the value.get 14.0 on Parser(\"1*2+3* 4\")" in {
    ExpressionParser("1*2+3* 4").flatMap(_.value).get should be(14)
  }

  it should "return the value.get 11.0 on Parser(\"1+2*3+4\")" in {
    ExpressionParser("1+2*3+4").flatMap(_.value).get should be(11)
  }

  it should "return the value.get 0.0 on Parser(\"1+4/2-3\")" in {
    ExpressionParser("1+4/2-3").flatMap(_.value).get should be(0)
  }

  it should "return the value.get 2.0 on Parser(\"2^1^2\")" in {
    ExpressionParser("2^1^2").flatMap(_.value).get should be(2)
  }
  ...
  it should "return the value.get 4.0 on Parser(\"(2^1)^2\")" in {
    ExpressionParser("(2^1)^2").flatMap(_.value).get should be(4)
  }

  it should "return the value 2Pi on Parser(\"2*PI\")" in {
    ExpressionParser("2*PI").flatMap(_.value).get should be(2 * math.Pi)
  }

  it should "return RPN 3 4 5 * - on 3 - 4 * 5" in {
    ExpressionParser("3 - 4 * 5").get.toString should be("3.0 4.0 5.0 * -")
  }
  ...
  it should "return RPN 3 4 - 5 * on (3 - 4) * 5 with Expression.apply" in {
    Expression("(3 - 4) * 5").get.toString should be("3.0 4.0 - 5.0 *")
  }

  it should "return 5 on \"x\" with x=5" in {
    Expression("x").flatMap(_.value(Map(Variable("x") -> Number(5)))).get should be(5)
  }

  it should "return 15 on \"2x +5\" with x=5" in {
    Expression("2x + 5").flatMap(_.value(Map(Variable("x") -> Number(5)))).get should be(15)
  }

  it should "return 15 on \"2x +5\" with x=2+3" in {
    Expression("2x + 5").flatMap(_.value(Map(Variable("x") -> Expression("2+3").get))).get should be(15)
  }
}