...
class ExpressionSpec extends FlatSpec with Matchers {
  ...
  it should "return the value.get 5 when it equals Plus(Number(2), Number(3))" in {
    Plus(Number(2), Number(3)).value.get should be(5)
  }

  it should "return the value.get 14 on Parser(\"1*2+3* 4\")" in {
    Parser("1*2+3* 4").value.get should be(14)
  }

  it should "return the value.get 11 on Parser(\"1+2*3+4\")" in {
    Parser("1+2*3+4").value.get should be(11)
  }

  it should "return the value.get 0 on Parser(\"1+4/2-3\")" in {
    Parser("1+4/2-3").value.get should be(0)
  }

  it should "return the value.get 2 on Parser(\"2^1^2\")" in {
  ...
  it should "return the value.get 4 on Parser(\"(2^1)^2\")" in {
